...
union EPDesc {
  ...
  // Default descriptor, in case eg: you need to initialize an array
  constexpr EPDesc() : EPDesc(USB_TRX_OUT, USB_EP_ATTR_CTL, USB_EP_SIZE) {}

  // Encode a direction, type, and max packet length in an endpoint descriptor.
...
class EPBuffer
{
  public:
    ...
    /* Busy loop until the endpoint has finished its current transmission */
